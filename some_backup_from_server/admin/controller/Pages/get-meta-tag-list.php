...
$aColumns = array('page_id', 'meta_title', 'meta_description', 'meta_keyword');
$sIndexColumn = "id";
$sTable = "meta_tag_lists";
/*
  * Paging
  */
$sLimit = "";
...
while ($aRow = $rResult->fetch_assoc()) {
  ...
  $query = "SELECT * FROM page_table WHERE id='" . $aRow['page_id'] . "'";
  $result = $con->query($query) or die("MetaTagList ERROR: " . mysqli_error($con));
  if (mysqli_num_rows($result) > 0) {
    $page_record = $result->fetch_assoc();
  ...
  $row[] = $page_name;
  $row[] = $aRow['meta_title'];
  $row[] = (strlen($aRow['meta_description']) > 50) ? substr($aRow['meta_description'], 0, 50) . '...' : $aRow['meta_description'];
  $row[] = $aRow['meta_keyword'];
  $row[] = '<div class="dropdown">
    ...
    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
      <a class="dropdown-item editMeta" href="javascript:void(0)" data-id="' . $aRow['id'] . '">Edit</a>
      <a class="dropdown-item deleteMeta" href="javascript:void(0)" data-id="' . $aRow['id'] . '">Delete</a>
    </div>
  </div>';
