...
$aColumns = array('customer_name', 'company', 'profile', 'key_point', 'description');
$sIndexColumn = "id";
$sTable = "testimonials";
/*
  * Paging
...
while ($aRow = $rResult->fetch_assoc()) {
    ...
    $row = array();
    if (!empty($aRow['profile'])) {
        $profile_img = $base_url . $aRow['profile'];
    } else {
        $profile_img = $base_url . 'assets/images/no-image.png';
    }

    $row[] = $aRow['customer_name'];
    $row[] = $aRow['company'];
    $row[] = '<a href="' . $profile_img . '" target="_blank"><img alt="customer" src="' . $profile_img . '" height="55" width="60"></a>';
    $row[] = $aRow['key_point'];
